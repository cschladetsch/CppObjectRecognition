...
TEST_F(AdvancedRectangleDetectionTest, HandlesDifferentAspectRatios) {
    ...
    for (const auto& rect : rectangles) {
        double aspectRatio = static_cast<double>(std::max(rect.width, rect.height)) / 
    ...
    for (double ratio : aspectRatios) {
        if (ratio > 3.5) hasWide = true;
        else if (ratio > 2.5) hasTall = true;
        else hasNormal = true;
    }
...
TEST_F(AdvancedRectangleDetectionTest, HandlesNoisyImages) {
    ...
    for (const auto& rect : rectangles) {
        if (std::abs(rect.center.x - 140) < 20 && std::abs(rect.center.y - 125) < 20) {
            foundMainRect = true;
            break;
        }
    }
    EXPECT_TRUE(foundMainRect) << "Should detect rectangle despite background noise";
}
...
TEST_F(AdvancedRectangleDetectionTest, DetectsOverlappingRectangles) {
    ...
    // Overlapping rectangles might merge into one complex shape that's not detected
    ...
    if (rectangles.size() > 0) {
        ...
        for (const auto& rect : rectangles) {
            if (rect.center.x > 50 && rect.center.x < 250 && rect.center.y > 50 && rect.center.y < 200) {
                foundReasonableRect = true;
                break;
            }
        }
        EXPECT_TRUE(foundReasonableRect) << "Detected rectangles should be in reasonable regions";
    }
}
...
TEST_F(AdvancedRectangleDetectionTest, RejectsIrregularPolygons) {
    ...
    EXPECT_LE(rectangles.size(), 1) << "Should not detect irregular polygons as rectangles";
}
...
TEST_F(AdvancedRectangleDetectionTest, PerformanceWithManyRectangles) {
    ...
    std::cout << "Detected " << rectangles.size() << " rectangles from " << rectCount 
              << " created in " << duration.count() << "ms" << std::endl;
}
