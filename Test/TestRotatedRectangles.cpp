...
TEST_F(RotatedRectangleTest, DetectsRectangleAt45Degrees) {
  ...
  ImageProcessor::CreateRotatedRectangle(image, 100, 100, 80, 50,
                                         std::numbers::pi / 4.0);
...
TEST_F(RotatedRectangleTest, DetectsMultipleRotatedRectangles) {
  ...
  std::vector<double> angles = {0.0, std::numbers::pi / 6, std::numbers::pi / 4,
                                std::numbers::pi / 3, std::numbers::pi / 2};
  ...
  std::cout << "Multiple rotated rectangles test - detected "
            << rectangles.size() << " rectangles:" << std::endl;
...
TEST_F(RotatedRectangleTest, DetectsRectanglesAtAllCommonAngles) {
  ...
  std::cout << "Detection rate across all angles: " << totalDetected << "/"
            << totalTested << " (" << (detectionRate * 100.0) << "%)"
            << std::endl;

  // We should detect at least 70% of rectangles across all angles
  EXPECT_GE(detectionRate, 0.7)
      << "Should have good detection rate across all angles";
}