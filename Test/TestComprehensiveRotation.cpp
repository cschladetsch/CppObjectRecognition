...
protected:
    void SetUp() override {
        ...
        detector.SetApproxEpsilon(0.015); // Tighter approximation for better corner detection
    }
    ...
    bool TestAngle(double angleDegrees) {
        ...
        ImageProcessor::CreateRotatedRectangle(image, 150, 150, 80, 50, angleRadians);
...
TEST_F(ComprehensiveRotationTest, TestEvery5Degrees) {
    ...
    EXPECT_GE(successRate, 70.0) << "Should detect rectangles at most rotation angles";
...
TEST_F(ComprehensiveRotationTest, TestProblematicAngles) {
    ...
    double problematicSuccessRate = (double)successCount / problematicAngles.size() * 100.0;
    std::cout << "\nProblematic angles success rate: " 
              << std::fixed << std::setprecision(1) << problematicSuccessRate << "%\n";
    
    // Should handle at least 50% of problematic angles
    EXPECT_GE(problematicSuccessRate, 50.0) << "Should handle most problematic angles";
}
...
TEST_F(ComprehensiveRotationTest, TestSteepAngleRange) {
    ...
    std::cout << "\nSteep angle range (60°-120°) success rate: " 
              << std::fixed << std::setprecision(1) << steepAngleRate << "%\n";
    
    // This is the critical test - can we handle steep angles?
    EXPECT_GE(steepAngleRate, 40.0) << "Should handle steep angles better than traditional methods";
}
...
TEST_F(ComprehensiveRotationTest, TestMultipleRectanglesAtDifferentAngles) {
    ...
    for (size_t i = 0; i < testAngles.size(); ++i) {
        ...
        ImageProcessor::CreateRotatedRectangle(image, centerX, centerY, 80, 50, angleRadians);
    }
    ...
    double multiDetectionRate = (double)rectangles.size() / testAngles.size() * 100.0;
    std::cout << "Multi-rectangle detection rate: " 
              << std::fixed << std::setprecision(1) << multiDetectionRate << "%\n";
...
TEST_F(ComprehensiveRotationTest, CompareWithBaseline) {
    ...
    std::vector<double> sampleAngles = {0, 15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165};
    int detectedCount = 0;
    ...
    double currentPerformance = (double)detectedCount / sampleAngles.size() * 100.0;
    
    std::cout << "Current system performance: " 
              << std::fixed << std::setprecision(1) << currentPerformance << "%\n";
    ...
    EXPECT_GT(currentPerformance, 40.0) << "Should outperform traditional contour-based methods";
}