...
TEST_F(RectangleDetectorTest, DiscriminatesNonRectangleShapes) {
    ...
    detector->SetApproxEpsilon(0.02);  // Reduce epsilon for better corner detection
...
TEST_F(RectangleDetectorTest, DetectsRectanglesAmongMixedShapes) {
    ...
    // Debug output
    std::cout << "Detected " << rectangles.size() << " rectangles in mixed shapes:" << std::endl;
    for (size_t i = 0; i < rectangles.size(); ++i) {
        std::cout << "  Rectangle " << i << ": center=(" << rectangles[i].center.x << "," 
                  << rectangles[i].center.y << "), size=" << rectangles[i].width << "x" 
                  << rectangles[i].height << ", angle=" << rectangles[i].angle << std::endl;
...
TEST_F(RectangleDetectorTest, OnlyDetectsCircles_ShouldFindZero) {
    ...
    ImageProcessor::DrawFilledCircle(testImage, 150, 60, 35, 255);  // Medium circle  
    ImageProcessor::DrawFilledCircle(testImage, 240, 60, 45, 255);  // Large circle
    ImageProcessor::DrawFilledCircle(testImage, 100, 140, 30, 255); // Another circle
...
TEST_F(RectangleDetectorTest, OnlyDetectsTriangles_ShouldFindZero) {
    ...
    ImageProcessor::DrawFilledTriangle(testImage, Point(50, 50), Point(100, 50), Point(75, 20), 255);
    ImageProcessor::DrawFilledTriangle(testImage, Point(150, 80), Point(200, 120), Point(120, 120), 255);
    ImageProcessor::DrawFilledTriangle(testImage, Point(250, 40), Point(320, 40), Point(285, 100), 255);
    ImageProcessor::DrawFilledTriangle(testImage, Point(80, 150), Point(140, 180), Point(60, 200), 255);
...
TEST_F(RectangleDetectorTest, OnlyDetectsEllipses_ShouldFindZero) {
    ...
    ImageProcessor::DrawFilledEllipse(testImage, 80, 80, 35, 20, 0.0, 255);      // Horizontal ellipse
    ImageProcessor::DrawFilledEllipse(testImage, 200, 80, 20, 40, 1.57, 255);    // Vertical ellipse  
    ImageProcessor::DrawFilledEllipse(testImage, 320, 80, 30, 18, 0.78, 255);    // Tilted ellipse
    ImageProcessor::DrawFilledEllipse(testImage, 150, 200, 45, 25, 2.35, 255);   // Another tilted ellipse
...
TEST_F(RectangleDetectorTest, DetectsOnlyRectanglesAmongMixedShapes) {
    ...
    ImageProcessor::DrawFilledTriangle(testImage, Point(350, 150), Point(420, 150), Point(385, 100), 255); // Triangle
    ImageProcessor::DrawFilledEllipse(testImage, 100, 300, 40, 25, 0.5, 255);    // Ellipse
    ...
    // Debug output
    std::cout << "Mixed shapes test - detected " << rectangles.size() << " rectangles:" << std::endl;
    for (size_t i = 0; i < rectangles.size(); ++i) {
        std::cout << "  Rectangle " << i << ": center=(" << rectangles[i].center.x << "," 
                  << rectangles[i].center.y << "), size=" << rectangles[i].width << "x" 
                  << rectangles[i].height << ", angle=" << rectangles[i].angle << std::endl;
