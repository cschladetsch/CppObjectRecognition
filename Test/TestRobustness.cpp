...
TEST_F(RobustnessTest, HandlesEmptyImage) {
    ...
    EXPECT_EQ(rectangles.size(), 0) << "Empty image should not detect any rectangles";
}
...
TEST_F(RobustnessTest, HandlesAllWhiteImage) {
    ...
    EXPECT_LE(rectangles.size(), 1) << "All white image should detect at most one rectangle";
}
...
TEST_F(RobustnessTest, HandlesSinglePixelShapes) {
    ...
    EXPECT_EQ(rectangles.size(), 0) << "Single pixels should not be detected as rectangles";
}
...
TEST_F(RobustnessTest, HandlesCheckerboardPattern) {
    ...
    EXPECT_GE(rectangles.size(), 10) << "Checkerboard should detect multiple rectangle squares";
    EXPECT_LE(rectangles.size(), 100) << "Should not detect excessive false positives";
}
...
TEST_F(RobustnessTest, HandlesGradientImages) {
    ...
    EXPECT_LE(rectangles.size(), 2) << "Gradient image should not detect many false rectangles";
}
...
TEST_F(RobustnessTest, HandlesImageWithHoles) {
    ...
    EXPECT_LE(rectangles.size(), 5) << "Rectangle with hole should not cause excessive detections";
}
...
TEST_F(RobustnessTest, HandlesRotatedRectangleAtExtremeAngles) {
    ...
    for (double angle : extremeAngles) {
        ...
        ImageProcessor::CreateRotatedRectangle(testImage, 200, 200, 100, 60, angleRad);
...
TEST_F(RobustnessTest, HandlesVeryThinRectangles) {
    ...
    // May or may not detect very thin rectangles depending on algorithm robustness
    EXPECT_GE(rectangles.size(), 0);
    EXPECT_LE(rectangles.size(), 2) << "Should handle thin rectangles without false positives";
}
...
TEST_F(RobustnessTest, HandlesAlmostRectangularShapes) {
    ...
    EXPECT_LE(rectangles.size(), 3) << "Should not detect too many false positives";
}
...
TEST_F(RobustnessTest, HandlesMultipleScales) {
    ...
    EXPECT_EQ(rectangles.size(), 3) << "Should detect rectangles of different scales";
...
TEST_F(RobustnessTest, StressTestWithRandomShapes) {
    ...
    EXPECT_GE(rectangles.size(), std::max(1, expectedRects / 5)) << "Should detect some rectangles in noisy environment";
    EXPECT_LE(rectangles.size(), expectedRects * 2) << "Should not have excessive false positives";
    
    std::cout << "Stress test: Created " << expectedRects << " rectangles, detected " 
              << rectangles.size() << std::endl;
}
...
TEST_F(RobustnessTest, HandlesIncompleteRectangles) {
    ...
    EXPECT_GE(rectangles.size(), 0);
    EXPECT_LE(rectangles.size(), 3) << "Incomplete rectangles should not cause excessive detections";
}